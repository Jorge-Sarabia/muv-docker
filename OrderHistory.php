...
class OrderHistoryCore extends ObjectModel
{
    ...
    public function changeIdOrderState($new_order_state, $id_order, $use_existing_payment = false)
    {
        ...
        // Code added for insert data in PostgreSQL from Prestashop
        if ($new_os->name == 'Pago aceptado' || $new_os->name == 'Payment accepted') {
            // Get the list of products in the order
            $products = $order->getProducts();

            // PostgreSQL database connection 
            try {
                $conn = new PDO("pgsql:host=postgres;port=5432;dbname=muv", "postgres", "postgres");
                $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
                echo "Connected successfully <br/>";

                // $insert_query = "INSERT INTO muv_pshop_signup_dev (nombre_asegurado, tel_asegurado, fecha_nac_asegurado, dir_asegurado, ciudad_asegurado, estado_asegurado, cp_asegurado, email_asegurado, nombre_beneficiario, tel_beneficiario, fecha_nac_beneficiario, dir_beneficiario, ciudad_beneficiario, estado_beneficiario, cp_beneficiario, email_beneficiario) ";
                // $query_values = "VALUES(";

                // // Go through the list of products and get the customized data fields and their values
                // foreach ($products as $product) {
                //   foreach ($product["customizedDatas"] as $customizedData) {
                //     foreach ($customizedData as $customizedFieldsData) {
                //       foreach ($customizedFieldsData as $fieldData) {
                //         foreach ($fieldData as $data) {
                //           for ($i = 0; $i < count($data); $i++) {
                //             echo $data[$i]["name"] . ': ' . $data[$i]["value"] . "<br />";
                //             $query_values .= "'" . $data[$i]["value"] . "', ";
                //           }
                //         }
                //       }
                //     }
                ...
                // $insert_query .= substr($query_values, 0, -2) . ");";

                // // Insert the data in $insert_query into the postgresql database 
        ...
        // End of code for PostgreSQL task
        ...
        if (Validate::isLoadedObject($order) && ($new_os instanceof OrderState)) {
            ...
            foreach ($order->getProductsDetail() as $product) {
                ...
                if (
                    $new_os->shipped == 1 && (!Validate::isLoadedObject($old_os) || $old_os->shipped == 0) &&
                    ...
                    $new_os->shipped == 0 && Validate::isLoadedObject($old_os) && $old_os->shipped == 1 &&
                    Configuration::get('PS_ADVANCED_STOCK_MANAGEMENT') &&
                    Warehouse::exists($product['id_warehouse']) &&
                    $manager != null &&
                    (int) $product['advanced_stock_management'] == 1
